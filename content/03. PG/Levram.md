...
tech_count: 5
---
PG Practice — Pentester Foundations #3
...
- "없다"를 입증하는 것은 "있다"보다 어렵다. 이 노트의 증거 B가 그 예다 — `~/PG/` 와 `/tmp/gv/` 에 wheel 이 없다고 해서 대조한 적이 없는 것은 아니었다. `pip` 는 받은 wheel 을 `~/.cache/pip` 에도 남긴다. **부재 증거를 존재 부정으로 승격시키지 마라**
...
증거 B 의 대조본이 `~/PG/` 와 `/tmp/gv/` 에는 남아 있지 않다는 점을 먼저 적어 둔다. `pip` 는 내려받은 wheel 을 HTTP 캐시(`~/.cache/pip/http-v2/…/*.body`)에도 남기므로, 그 캐시에서 wheel 3종을 꺼내 전수 재대조했고 아래 서술이 항목별로 전부 일치했다. 재현 절차는 이 절 끝에 붙였다.
...
이 절과 1-2·1-4의 `curl` 블록은 재현 절차로 읽어라 `[가정]`
...
> [!danger] "비인증으로 열린 것은 `/api/user/auth` 하나뿐"은 거짓이다 — 놓치기 쉬운 대안 경로
> 0.9.7 `views.py` 를 직접 세어보면 `@api_view` 42개 중 두 곳에서 권한 데코레이터가 주석 처리돼 있다:
> ```bash
> ┌──(kali㉿kali)-[~/PG/Levram]
...
반대 방향의 위험도 하나 있다. 하지 않은 대조를 근거로 세지 마라. 확보한 축이 하나면 하나라고 적고, 적었으면 재현 절차를 함께 남긴다. 증거 B를 지켜준 것이 그 재현 가능성이었다 — 캐시에 남은 wheel 로 전항목이 다시 나온다.
...
브라우저에서도 차이는 보인다. 다만 슬래시를 붙였을 때만 보인다 — 0.9.7은 어느 쪽으로 때려도 결국 살고, 0.9.8은 슬래시를 붙이면 죽는다. "브라우저로는 안 보인다"고 뭉뚱그릴 자리가 아니다. 그래도 가장 확실한 관측 지점은 404 본문에 덤프된 정규식 원문이다 — 리다이렉트 동작은 커스텀 미들웨어·프록시가 끼면 흐려지지만 덤프된 정규식은 서버 코드 그 자체다.
...
`[유용]` 반대로 wheel 은 `-d` 디렉터리에서 지워도 `~/.cache/pip/http-v2/` 에 `.body` 로 남는다. 나중에 "받은 적 있었나"를 되짚을 때 1차 사료가 된다 — 이 노트의 증거 B 대조본이 그 캐시에 남아 있었다(1-2 재현 절차).
...
> [!warning] `DEBUG=True` 가 `ALLOWED_HOSTS` 를 느슨하게 만든다는 것은 거짓이다 — 위 표에 네 번째 행으로 넣고 싶어지는 자리
...
이고 `spider_name` 이 명령 문자열의 맨 끝이다. 뒤에 남는 인자가 없으므로 `;`·`&&`·개행도 그대로 통한다. 실행 파일도 `scrapy` 가 아니라 `gerapy` 다 — 제품이 Scrapy 관리 UI 라고 해서 명령줄까지 `scrapy` 인 것은 아니다. 그럼에도 페이로드가 백틱인 이유는 문법 때문이 아니라 EDB 50640이 그렇게 만들어져 있어서 그대로 따랐기 때문이다.
...
> [!warning] ①을 "DRF가 `request.data` 로 꺼낸다"고 적으면 틀린다
...
> [!danger] "프로젝트 생성이 선행조건"은 거짓이다
> 그럴듯한 근거는 "프로젝트가 0개면 뷰가 프로젝트 디렉터리를 찾다가 명령 조립 지점에 도달하기 전에 실패한다"는 것이다.
...
> 이 노트가 미끄러진 지점이 여기다. 스크립트가 죽는 것을 보고 취약점 쪽에 없는 전제조건을 만들어 붙이면, 그 허구의 전제조건이 상단 요약·3장·6장 ④·7-1 까지 서로 멀리 떨어진 절로 함께 번진다.
...
> [!warning] `-H 'Content-Type: application/json'` 은 이 엔드포인트에서는 없어도 된다 — "빼면 400"이 통하지 않는 자리
> "빼면 400"은 DRF가 바디를 파싱한다는 전제 위에 선다. 그런데 `project_create` 도 `project_parse` 도 `json.loads(request.body)` 로 원시 바디를 직접 읽는다(2-4). `request.data` 를 건드리지 않으므로 파서가 발동하지 않고 Content-Type 은 무관하다.
>
> 반증 사례가 이 노트 안에 있다 — EDB 50640은
> ```python
> r5 = r.post(url + "/api/project/" + str(id) + "/parse", data=payload, headers=auth_token)
> ```
...
원문에 기록이 남은 것은 `id`·`getcap`·`app.service` 셋이다
...
> [!warning] "SUID는 복사해도 대개 유지된다"는 것은 거짓이다
> Kali에서 그대로 때려본 결과다:
> ```bash
...
> [!warning] linpeas 는 시험 허용 도구다 — 기록에서 지울 이유가 없다
> `linpeas`/`LinEnum`/`pspy` 는 열거 전용이라 OSCP에서 허용된다([[_WRITEUP-STANDARD]] 참조).
> 규정이 걸려 실제로 쓴 도구를 목록에서 빼고 그 자리에 돌리지 않은 `getcap -r /` 출력을 채워 넣으면, 다음 세션이 "이 박스는 수동 열거로 풀렸다"고 오판한다. 규정 문제가 아니라 기록 문제다.
...
> [!danger] `Password:` 뒤가 비어 있는가 아닌가는 pty 유무로 갈린다
...
>
> > [!warning] "`su` 는 에코를 끄므로 화면에 절대 안 나온다"는 **절반만 맞다**
> > **TTY 가 없어도 `su` 는 stdin 에서 비밀번호를 읽는다**([[plum]] 실측) — 그래서 pty 없이 치면 화면에 남는다.
> > 즉 `su root` 블록에 비밀번호가 찍혀 있다면 **두 해석이 모두 가능하다**:
> > ① 노트를 쓰면서 가독성을 위해 덧붙인 것 — ② **pty 업그레이드 없이 `su` 를 쳤다는 증거**(로컬 에코).
> > `[가정]` **이 박스에서 어느 쪽인지는 확정할 수 없다.** 원본 캡처가 남아 있지 않고, `pathB.sh` 는 `tmux send-keys -l` 로 밀어 넣어 재현해도 로컬 에코 여부가 당시와 같다는 보장이 없다.
...
> [!warning] ~~`su`는 TTY를 요구한다~~ — 반증됨 ([[plum]] 실측)
> "업그레이드 안 된 리버스셸에서 `su`를 치면 `su: must be run from a terminal` 이 난다"는 규칙이 흔히 통용되지만, [[plum]]에서 TTY 없이도 `su` 가 stdin 에서 비밀번호를 읽고 인증에 성공하는 것이 실측됐다. 반사적으로 믿을 규칙이 아니다.
...
> SSH 세션 기록이 산출물 어디에도 없다. `proof.txt` 값은 위 `su root` 경로에서 얻은 것이다.
>
...
> [!danger] 여기서 한 걸음 더 나가면 노트가 통째로 오염된다
> 여기까지는 정확하다. 위험한 것은 "그러니까 프로젝트 생성은 취약점의 선행조건"으로 승격시키는 것이다.
> 취약 코드에는 그런 검사가 없다(2-6). 스크립트의 성질을 취약점의 성질로 옮겨 적는 것이고, 그런 승격은 한 번이면 상단 요약·2-6·3장·6장 ④·7-1 처럼 멀리 떨어진 절까지 함께 번진다.
...
이 관측의 대조본은 `pip` 캐시에 남아 있다
`~/PG/` 와 `/tmp/gv/` 에는 정품 wheel 이 남아 있지 않지만, `pip` 의 HTTP 캐시(`~/.cache/pip/http-v2/…/*.body`)에 그대로 있다. 캐시 wheel 로 재대조하니 위 수치가 전부 맞았다(1-2 재현 절차).
비용은 정품 wheel 확보와 크기·문자열 대조로 몇 분. 판정에 필요한 작업은 아니었고(증거 A에서 이미 끝나 있었다) 부산물이 값졌을 뿐이다.

남는 교훈은 둘이다. 조작 가능한 단일 근거로 판정하지 않는다는 것 — 이 박스는 그 규율이 옳았음을 사후에 증명해준 드문 사례이고, 보통은 조작 여부를 영영 모른 채 넘어간다. 그리고 "조작됐음을 확인했다"고 쓰려면 대조본과 재현 경로를 함께 적어야 한다는 것. 대조를 해놓고 경로를 안 적으면 나중에 스스로도 그 대조를 증명하지 못한다. **결론만 적힌 관측은 자기 자신을 방어하지 못한다.**
...
- **순서 주의**: 산출물의 호출 순서는 `setuid` 가 먼저다. 순서와 무관하게 `setgid` 호출에서 막힌다
...
- **소득이 아니었던 것**: "SSH 재접속 경로"는 여기 넣을 수 없다. SSH는 시도하지 않았다(4-4). 얻은 것은 비밀번호이지 검증된 채널이 아니다 `[가정]`
...
| 두 번째 시도부터 리버스셸이 안 붙음 | 앞선 요청의 핸들러가 셸을 물고 있다 | 리스너 포트를 바꿔 재시도 `[가정]` — 이 박스에서 겪은 일이 아니다. 산출물(`trigger.sh`)과 이전 세션 기록 모두 4444 하나뿐이다 |
...
> `linpeas.sh` 는 열거 전용이라 허용된다 — 익스플로잇 단계가 없다. 실제로 privesc를 짚어준 것이 linpeas 이므로(4-3) 도구 목록에서도 프론트매터의 `tech/enum/peas` 태그에서도 뺄 이유가 없다. 규정 문제가 아니라 기록 문제다.
...
"② 프로젝트 생성 (선행조건)"을 끼워 넣어 4개로 적고 싶어지는 자리다
...
1. 버전 판정의 목표는 "정확한 버전"이 아니라 "패치 경계의 어느 쪽인가"다. 여기서는 런타임 동작 차이 하나(URLconf 덤프의 트레일링 슬래시)로 후보가 `{0.9.5, 0.9.6, 0.9.7}` 로 좁혀졌고 셋 다 취약이라 거기서 끝났다. 근거를 더 쌓아야 하는 것은 후보 집합이 패치 경계를 걸칠 때뿐이다.
   ...
   그리고 이 노트가 비싸게 배운 규율 하나 — 한 대조는 재현 절차와 함께 적는다. 증거 B는 결론만 적혀 있어서 나중에 되짚을 때 `~/.cache/pip` 에 남은 wheel 을 찾아내기 전까지는 스스로를 증명하지 못했다. 재현 경로가 없는 관측은 자기 자신을 방어하지 못한다.
...
3. "인증 후 RCE" CVE를 봤으면 기본 자격증명부터 시도한다. 인증 요구는 CVSS를 낮추지만 현실에서는 장벽이 아니다. `admin:admin` 이 `parse` 로 가는 문을 열었다.
   덧붙여, "나머지는 전부 401이더라"는 응답을 세어서 내릴 결론이 아니다. 오픈소스 제품이면 `grep -n 'permission_classes' views.py` 한 줄이 전수 조사이고, 이 박스에서는 그렇게 하자 주석 처리된 데코레이터 2개(`index`·`project_upload`)가 나왔다. 확인 없이 "비인증은 `/api/user/auth` 하나뿐"이라고 단정하기 딱 좋은 자리였다.
...
12. **"내가 찾아본 곳에 없다"는 "존재하지 않는다"가 아니다.** 이 노트의 증거 B가 그 함정 위에 놓여 있다 — `~/PG/` 와 `/tmp/gv/` 두 디렉터리만 보면 "wheel 을 받은 적이 없다 → 대조한 적이 없다"로 두 단계를 뛰게 되는데, wheel 은 `~/.cache/pip/http-v2/` 에 `.body` 로 멀쩡히 있었다.
    ...
    기록을 되짚을 때만의 규율이 아니다 — 침투 중 "이 시스템에 X는 없다"고 단정하는 것도 같은 오류다. 캐시와 히스토리는 항상 뒤진다.
...
- [[Hub]] — 버전 판정을 같은 출처 근거 3개로 하다 틀린 사례. 이 박스의 방법론이 나온 배경이고, "출제자가 표식을 조작한다"는 교훈의 실제 근거지
...
- [[plum]] — `su` 가 TTY 없이도 동작한다는 실측이 나온 박스. 이 노트 4-4 의 `su` 서술이 그 실측을 따른다
