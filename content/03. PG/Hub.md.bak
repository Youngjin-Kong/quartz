...
>
> 반대로 **CVE 번호 암기는 거의 무가치**하다. 이 박스만 해도 랩 브리핑은 **8.1**, 실행 중인 것은 **8.4**로 갈렸지만 악용 절차는 그대로였고, 나중에 이 노트가 붙였던 CVE 번호(`CVE-2024-27697`)마저 **틀린 것으로 판명**났다(1장 참조). **그동안에도 체인은 계속 통했다.**
...
- **9999 = 같은 앱의 HTTPS 리스너.** `curl -k https://T:9999/`가 8082와 동일한 Home을 반환한다. `bdd.conf`의 `port=8082 / sslport=9999`가 확증.
  - ⚠️ nmap이 `ssl/abyss?`라고 쓴 것은 **오인이 아니다.** `abyss`는 `/usr/share/nmap/nmap-services`에 `abyss 9999/tcp 0.004441`로 등재된 **9999/tcp의 표준 서비스 이름**(Abyss Web Server 관리 인터페이스)이고, nmap은 지문이 안 맞으면 **포트 번호 기반 기본 라벨**을 붙인다. 꼬리의 **`?`가 "버전 지문 미확정"** 표시다.
  - **읽는 법: `서비스명?`의 `?`를 보면 "nmap이 추측만 했다"는 뜻이다.** 서비스명을 사실로 받지 말고 직접 붙어서 확인한다. `?`가 없으면 지문이 실제로 매칭된 것이다.
...
> | 플래그 | 역할 | **빼면 어떻게 실패하나** |
> |---|---|---|
> | `-p-` | 65535 포트 전수 | ⚠️ **이 박스에서는 없어도 된다.** `8082`(`blackice-alerts`)와 `9999`(`abyss`)는 **둘 다 `nmap-services`에 등재돼 top-1000 안**이다. 아래 실측 참조. `-p-`의 가치는 이 박스가 아니라 [[Hawat]](50080)처럼 **등재되지 않은 고번호**에서 나온다 |
...
> [!danger] 정정 — `-p-`가 이 박스를 살렸다는 서술은 **같은 세션의 자기 로그가 반증한다**
...
>
> **그럼 `-p-`를 쓰지 말라는 뜻인가? 아니다.** 습관 자체는 옳다. 틀린 것은 **근거**다.
> - `-p-`가 실제로 결정적인 경우: **`nmap-services`에 없거나 빈도가 극히 낮은 고번호 포트.** [[Hawat]]의 50080이 그 예다
> - 이 박스에서 `-p-`가 준 실익: **65535 전수를 확인해 "더는 없다"를 확정**한 것. 발견이 아니라 **배제**다
>
> > [!warning] 이것이 이 노트의 가장 비싼 종류의 오류다
> > **좋은 습관을 가르치려고 틀린 근거를 댔다.** 그 근거를 믿은 독자는 다른 박스에서 **top-1000을 과소평가**하게 되고, "빠른 스캔은 의미 없다"며 매번 `-p-` 완료를 기다리다 시간을 태운다.
> > [[_WRITEUP-STANDARD#단정형 일반 지식은 반드시 때려보고 넣는다|표준의 「단정형 일반 지식」 절]]이 이 사례를 인용하고 있다.
> > **일반화의 근거는 "이 박스의 관측"이 아니라 "별도 확인"이어야 한다.** 여기서는 `grep 8082 /usr/share/nmap/nmap-services` 한 줄이면 끝났다.
...
> [!danger] 정정 — 두 CVE는 "같은 결함을 공유"하지 않는다. **별개 취약점이다**
> 이 노트는 원래 *"이 체인은 8.1의 CVE-2023-24078(EDB 51550)과 8.4의 CVE-2024-27697이 공유하는 결함이다"* 라고 단정했다. **성립하지 않는다.**
>
> | CVE | 무엇인가 | 근거 |
> |---|---|---|
> | **CVE-2023-24078** | *"Real Time Logic FuguHub **v8.1 and earlier** … remote code execution (RCE) vulnerability via the component **/FuguHub/cmsdocs/**"* — **파일 업로드/WebDAV 계열.** 이 노트가 실제로 탄 경로가 이쪽이다 | NVD (CVSS 3.1 8.8 HIGH, CWE-94) |
> | **CVE-2024-27697** | FuguHub **8.4**에서 관리자 패널 About 페이지(`customize.lsp`)가 **편집 가능한 Lua 페이지**라는 점을 악용해 Lua 리버스셸을 주입하는 **별개** 취약점 `[가정 — NVD 미공개(`totalResults: 0`)이며 공개 PoC 설명 기준]`. 그 PoC조차 *"계정 생성/인증에는 **CVE-2023-24078**을 leverage 하고 그 다음 `customize.lsp`"* 라고 **두 CVE를 구분해서** 쓴다 | NVD REST API 조회 시 **결과 0건** |
>
> **결론: 이 노트가 실제로 탄 것은 CVE-2023-24078 계열이다.** 무인증 관리자 선점 + WebDAV 업로드 RCE. `customize.lsp`는 이 노트에서 건드린 적이 없다.
> 프론트매터 `cves:`에서도 `CVE-2024-27697`을 제거했다.
>
> > [!warning] 남의 CVE 번호를 의심할 때는 자기 번호도 같은 기준으로 확인하라
> > 6장 ②는 **"랩 브리핑의 CVE 번호가 틀렸다"고 지적하는 장**이다. 그런데 **그 지적이 제시한 대체 번호가 또 틀렸다.**
> > 왜 이렇게 되나 — **버전이 맞으면 CVE도 맞을 것이라는 추론**이 그럴듯하기 때문이다("실행 버전이 8.4니까 8.4용 CVE가 맞겠지"). **버전 인접성은 취약점 동일성의 근거가 아니다.**
> > **확인 비용은 30초다** — NVD 상세 페이지를 열어 **description 한 문장**을 읽으면 어느 컴포넌트인지 나온다. 그것이 내가 탄 경로와 같은지만 보면 된다.

**CVE 번호를 맞히는 것보다 메커니즘을 아는 것이 실전에서 중요하다** — 이 박스가 그 증거다. 번호를 두 번 틀리는 동안에도 체인은 그대로 통했다.
...
> [!warning] 정정 — `/rtl/about.lsp`는 원래 이 표에 **무인증 200**으로 적혀 있었다
...
>
> **무인증 200이 맞다면 이 노트 최대 학습 자산인 2-6장의 교훈 자체가 무너진다** — 버전을 30분 헤맨 것이 "인증이 없어서 못 봤다"가 아니라 **"열려 있는 페이지를 안 열어봤다"** 가 되기 때문이다. 그건 인식론 문제가 아니라 단순 열거 누락이다.
>
> 다수 근거를 따라 표를 **`401 → 인증 후 200`으로 정정**했다. 다만 **타겟이 정지돼 재확인이 불가**하므로 `[가정]`으로 둔다. 이 박스를 다시 리버트할 기회가 있으면 **가장 먼저 확인할 한 줄**이 이것이다:
> ```bash
> curl -s -o /dev/null -w "%{http_code}\n" http://TARGET:8082/rtl/about.lsp
> ```
> **교훈: 표와 서술이 어긋나면 그중 하나는 반드시 틀렸다.** 표는 나중에 요약하면서 쓰기 때문에 특히 잘 어긋난다 — **표를 먼저 의심하라.**
...
> [!warning] "디렉터리가 전부 403"이 아니다 — **6개 중 4개는 301**이다
> 이 노트는 원래 *"`/applications/ /data/ /disk/ /themes/ /cache/ /trace/` **403** (autoindex 꺼짐)"* 이라고 한 줄로 뭉뚱그려놨었다. 로그가 반박한다:
>
> | 코드 | 경로 | 뜻 |
> |---|---|---|
> | **403** | `/`, `/cache`, `/trace` | 접근 거부. 153바이트 nginx 기본 에러 페이지 |
> | **301** | `/data`, `/themes`, `/applications`, `/disk` | **후행 슬래시 리다이렉트**(nginx가 디렉터리를 인식하고 `/path` → `/path/`로 보냄). 169바이트 |
>
> **차이가 실전에서 중요하다.** 301은 **"이 디렉터리가 실재한다"는 확정**이다(nginx는 존재하지 않는 경로에 301을 주지 않는다). 403은 그다음 단계 — 존재하고, 인덱싱만 막혔다는 뜻이다.
> **즉 `/data`·`/disk`·`/applications`는 "실재하는 디렉터리"로 확정된 상태**였고, 그 안의 파일명을 노려야 할 곳이었다. "전부 403"으로 뭉뚱그리면 이 신호가 사라진다.
> **일반화: 스캔 결과를 요약할 때 상태코드를 합치지 마라.** 301/403/401/200은 각각 다른 다음 수를 지시한다.

아래 파일들은 실제로 80에서 내려받았고(로컬에 보존돼 있다) 이 박스의 정보 유출 본체다 — **다만 위 스캔이 찾아준 것이 아니다**(⑤ 참조):
...
`user.dat`(467바이트)은 **바이너리가 아니라 한 줄짜리 JSON**이다:
...
키 하나(`v`)에 **base64 덩어리 하나**가 들어 있다. 풀어보면 알려진 해시 접두사(`$1$`·`$6$`·`$apr1$`)가 없는 **불투명 blob**이라 크랙 대상이 아니다(상세 판정 절차는 6장 ⑥). 그래도 **설치 경로 확정**과 **셸 획득 후 드롭한 파일을 80으로 회수**하는 용도로 쓸 수 있다.
...
> [!danger] 확장자 화이트리스트를 좁게 잡으면 설정 파일이 통째로 안 보인다 — **그런데 이 박스는 그 이야기가 아니었다**
> 아래 feroxbuster는 `-x php,txt,html,lsp`로 돌렸다. **이 목록에 `conf`·`dat`이 없다.** 그래서 `bdd.conf`·`user.dat`이 그 스캔 결과에 없는 것은 사실이다.
>
> **그런데 확장자를 넓힌 2차 스캔도 못 찾았다.** `~/PG/Hub/ferox_80_files.log`를 열면 실제로 `-x conf,dat,zip,log,bak,txt`로 돌렸는데 결과가 **세 줄뿐**이다:
> ```
> 403  /            200  /LICENSE.txt (87c)      200  /readme.txt (18730c)
> ```
> **진짜 원인은 확장자가 아니라 워드리스트다.** `raft-medium-files.txt`에는 `bdd`도 `user.dat`도 **들어 있지 않다**(전수 grep 0건). 확장자를 아무리 붙여도 **기저 단어가 없으면 요청 자체가 발생하지 않는다.**
>
> > [!warning] 브루트포스의 실패는 두 축으로 분해된다 — 어느 쪽인지 구분하라
> > | 축 | 증상 | 대응 |
> > |---|---|---|
> > | **확장자** 부족 | 파일명은 워드리스트에 있는데 못 찾음 (`config` 있으나 `config.conf` 미시도) | `-x`를 넓힌다 |
> > | **워드리스트** 부족 | 파일명 자체가 목록에 없음 (`bdd`·`user.dat`) | **워드리스트를 바꾸거나, 제품 지식·소스에서 파일명을 가져온다** |
> >
> > 후자는 `-x`를 아무리 늘려도 안 풀린다. **브루트포스로 안 나오는 파일명은 "제품이 무엇을 쓰는지"에서 온다** — 벤더 문서, 배포 아카이브(이 박스는 `/applications/Config-Wizard.zip`이 통째로 받아진다), 설치 가이드, GitHub 소스.
> > **제품이 특정된 순간 브루트포스의 우선순위는 내려가고 제품 지식의 우선순위가 올라간다.** [[Crane]] 1장에서 같은 결론에 도달했다.
>
> 아래 2단 확장자 전략은 **여전히 옳은 습관**이다(다른 박스에서 실제로 작동한다). 다만 **"이 박스에서 그 방법으로 `bdd.conf`를 찾았다"는 인과는 성립하지 않는다** `[가정 — 실제로는 제품 지식이나 `Config-Wizard.zip`에서 왔을 가능성이 높으나 그 과정의 산출물이 남아 있지 않다]`.
>
...
>
> 실전 규칙: **1차는 좁게 빠르게, 2차는 설정·데이터 확장자로 다시. 그리고 2차도 비면 워드리스트를 의심하라.**
...
| `email=` 이메일 형식 문자열 | 계정 메타데이터 | **비어 있으면 폼 검증에 걸릴 수 있다.** 형식만 맞으면 **무엇이든 통과**한다 — 아래 참조 |
...
> ```
>
> 인증서 SAN이 실제로 준 것은 **`FuguHub`·`FuguHub.local`·`localhost`** 셋이다. 위 요청에 쓴 값은 박스 이름에서 유추해 **임의로 지은 것**이고, **이 필드는 형식 검증만 통과하면 되므로 아무 값이나 무방**하다(`a@b.c`로도 된다 — 3장 수동 대안 4줄이 그렇게 쓴다).
> 이 박스에서 `/etc/hosts`에 등록할 후보로 삼을 만한 값은 **`FuguHub.local`** 하나뿐이다.
>
> **왜 이 오류가 위험한가**: 같은 노트 1장이 *"`DNS:FuguHub.local`처럼 내부 도메인이 새기도 한다 — `/etc/hosts`에 등록해두면 vhost 기반 라우팅을 놓치지 않는다"* 고 **정확히** 조언한다. 그런데 2-3장은 인증서에 없는 값을 "인증서에서 얻었다"고 썼다.
> **인증서에서 얻은 도메인은 `/etc/hosts`에 등록할 실제 후보**다. 지어낸 값을 섞어 적으면 그 목록이 오염된다 — 다음 박스에서 vhost를 헛짚게 만든다.
> **규칙: 내가 지어낸 값과 타겟이 준 값을 절대 섞어 쓰지 마라.** 프론트매터 `domain:` 필드도 그래서 제거했다(이 박스는 확인된 vhost 도메인이 없다).
...
그런데 FuguHub의 LSP 런타임은 **`io` 테이블에서 `popen`을 제거**해뒀다. 호출하면 "`popen`이라는 필드는 함수가 아니다"라는 취지의 에러로 죽는다.
...
>
> **왜 이게 특히 나쁜 오류였나**: 이 절(2-5)은 **"에러 문구를 정확히 읽어라"고 가르치는 장**이다. 가르치는 문구 자체가 부정확하면 교훈이 통째로 무너진다. 게다가 틀린 문구를 그대로 구글에 넣으면 **검색 결과가 0건**이 나와 "희귀한 커스텀 런타임인가" 하고 엉뚱한 방향으로 간다.

> [!note] 그래도 **의미로 분류하는 법**은 그대로 유효하다 — 이게 실전 자산이다
> | 에러 계열 (표준 Lua 기준) | 뜻 | 다음 수 |
> |---|---|---|
> | `attempt to call a nil value (field 'popen')` | **그 필드가 nil이다.** 샌드박스가 함수를 걷어냈다 | **다른 API를 찾아라** — 제품 확장 API 포함 |
> | `attempt to index a nil value (global 'io')` | **`io` 테이블 자체가 없다.** 더 강한 샌드박스 | 전역 테이블부터 열거한다 |
> | `permission denied` / 빈 출력 / rc≠0 | **함수는 살아 있는데 OS·정책 레벨에서 막힌다** | **다른 실행 주체를 찾아라** — AppArmor·seccomp·`open_basedir` 계열 의심 |
>
> 셋은 대응이 완전히 다르다. 에러를 대충 보고 "샌드박스라서 안 되네"로 뭉뚱그리면 잘못된 방향으로 간다.
> **런타임이 표준이 아니면 문구도 다르다.** 그러니 **문구가 아니라 "무엇이 nil인가 / 무엇이 거부됐는가"를 읽어라.**
...
| 수단 | 명령 | 비고 |
|---|---|---|
| **About/버전 페이지** | `curl -s -u U:P http://T:P/rtl/about.lsp \| grep -i <제품명>` | 최상위. 인증이 필요할 때가 많으므로 **자격증명 확보 후 즉시** 확인 `[가정 — 이 박스에서 `/rtl/about.lsp`가 무인증으로도 열렸는지는 미확정이다. 1장 표 참조]`. **무인증으로도 열리는지 먼저 한 번 때려보는 것이 순서상 먼저다** — 열려 있으면 자격증명을 기다릴 이유가 없다 |
...
> | 업로드 `/fs/t1.lsp` → 실행 `/t1.lsp` | **경로가 다르다** | `/fs/`는 WebDAV 파일 매니저의 네임스페이스이고, 실행은 **웹 루트 경로**에서 확인했다. `/fs/` 쪽으로 GET했을 때 무엇이 오는지는 **이 박스에서 확인하지 않았다** `[가정 — 파일 매니저가 소스를 그대로 돌려줄 가능성이 높다]` |
>
> 마지막 줄이 이 단계의 핵심 함정이다. **업로드 경로와 서빙 경로를 분리해서 생각하라.**
> ⚠️ 이 노트는 원래 *"`/fs/t1.lsp`를 GET하면 **파일 매니저가 소스를 그대로 돌려준다**(실행 안 됨)"* 를 **실측처럼** 단정했으나, 원본·산출물 어디에도 `/fs/x.lsp`를 GET한 기록이 없다(6장 ⑦도 같은 항목을 "실측 기록 없음" 표에 넣어놨다 — **내부 모순**이었다). 확인된 것은 **웹 루트 경로에서 실행됐다**는 것뿐이다.
> **다음에 이 유형을 만나면 반드시 양쪽을 다 때려봐라** — `GET /fs/x.lsp`와 `GET /x.lsp`를 나란히 놓고 응답을 비교하는 것이 30초짜리 확정 절차다.

`201 Created` → **서버가 새 리소스를 만들었다는 확답.** 덮어쓰기였다면 `204 No Content`가 온다. 이 코드 차이로 **파일이 이미 존재했는지**까지 알 수 있다.

### Step 3 — `io.popen` 차단 → `ba.exec()`

교과서적 Lua 웹셸은 `io.popen`을 쓰는데 FuguHub의 LSP 샌드박스는 이걸 제거해놨다 — **`io.popen`이 nil이라 호출 자체가 실패**한다(표준 Lua라면 `attempt to call a nil value (field 'popen')`, 제품 런타임에 따라 문구는 다를 수 있다. 2-5 참조).
...
> 위 유닛 파일은 **원본 노트에서부터 있던 블록**이지만(개작이 지어낸 것은 아니다), `~/PG/Hub/`에 대응하는 산출물이 없어 **원문 대조가 불가능**하다. 그리고 몇 가지가 제품 관례와 어긋난다:
>
> | 어긋나는 점 | 왜 이상한가 |
> |---|---|
> | `ExecStart=/var/www/html/FuguHub` | 같은 박스에서 회수한 설정 파일이 **`bdd.conf`**(**B**arracuda**D**rive **D**aemon)이다. FuguHub/BarracudaDrive 계열의 데몬 실행 파일은 통상 **`bd`** 다 |
> | 유닛명 `fuguhub.service` | 위와 같은 이유로 `bd.service`류일 가능성이 있다 |
>
> **결론이 바뀌지는 않는다** — `ba.exec('id')`가 실측으로 `uid=0(root)`을 돌려줬으므로 **서비스가 root로 돈다는 사실 자체는 확정**이다. 불확실한 것은 **유닛 파일의 정확한 내용과 이름**뿐이다.
>
...
> ```
> **교훈: 권한상승 장의 "왜 그런가"를 설명하는 설정 파일은 반드시 원문을 파일로 남겨라.** 셸이 끊기면 되찾을 수 없고, 기억으로 재구성한 설정 파일은 **그럴듯하게 틀린다.**
...
>
> **이 박스는 세 번째가 아니다.** 위에 인용한 유닛에는 `User=root`가 **명시**돼 있다 — 빠뜨린 것이 아니라 **의도적으로 root를 지정**한 것이다. FuguHub가 8082/9999만 쓰는데도 그렇다는 점에서 **첫 번째(파일 서버 기능)** 쪽으로 보인다 `[가정]`.
> 세 번째는 **일반론**이다. 그 자체는 여전히 맞다 — **`User=` 줄이 없는 유닛 파일은 root로 돈다**는 것이 systemd 시스템 인스턴스의 기본값이고, "설정 실수"의 대표형이다.
> **다만 이 박스를 그 사례로 인용하지 마라.** 명시된 `User=root`와 생략된 `User=`는 **보고서에서 지적할 문장이 다르다** — 전자는 "설계 결정을 재검토하라", 후자는 "명시하지 않아 기본값이 적용됐다".
...
2. **절차적으로**: **자격증명을 확보한 직후 가장 먼저 할 일이 About/버전 페이지 확인**이다. 무인증 상태에서 내린 버전 판정은 전부 잠정으로 취급하고, 인증이 열리는 순간 재검증한다.
   `[가정 — 이 박스에서 `/rtl/about.lsp`에 인증이 필요했는지가 미확정이다.` 1장의 무인증 표면 표는 한때 이 경로를 **무인증 200**으로 적어놨고, 본문 네 곳은 **인증 후 접근**을 전제한다. 다수 근거를 따라 표를 정정했으나 타겟이 정지돼 재확인이 불가하다. **만약 무인증으로도 열렸다면 이 교훈은 인식론 문제가 아니라 단순 열거 누락**이 된다]`
   **그래서 실전 순서는 이렇게 잡아라 — ① 무인증으로 About을 먼저 때려본다 → ② 401이면 자격증명 확보 후 즉시 재시도.** ①을 건너뛰면 "인증이 필요했다"고 착각한 채 30분을 태울 수 있다.
...
랩 문서는 이 박스를 **8.1 / CVE-2023-24078**로 안내한다. 실행 중인 버전은 **8.4**다.

> [!danger] 이 절이 제시했던 "정답 CVE"도 틀렸다 — 이중 오류
> 이 노트는 원래 여기서 *"CVE도 8.4 계열이면 **CVE-2024-27697**이 맞다"* 고 썼다. **아니다.**
> - **실제로 탄 경로**(무인증 관리자 선점 + WebDAV `.lsp` 업로드 RCE)는 **CVE-2023-24078** 쪽이다 — NVD 설명이 `/FuguHub/cmsdocs/` 컴포넌트를 통한 RCE라고 명시한다
> - **CVE-2024-27697**은 관리자 패널 About 페이지(`customize.lsp`)를 편집해 Lua를 주입하는 **별개** 취약점이고 `[가정 — NVD 미공개]`, 이 노트는 `customize.lsp`를 건드린 적이 없다
> - 즉 **랩 브리핑의 CVE 번호가 오히려 맞았고**, 틀린 것은 **버전**뿐이었다
>
> **왜 이렇게 됐나** — *"실행 버전이 8.4니까 8.4용 CVE가 맞겠지"* 라는 추론이 그럴듯했기 때문이다. **버전 인접성은 취약점 동일성의 근거가 아니다.**
> **이 절의 원래 교훈("힌트를 사실로 취급하지 마라")은 그대로 유효하다. 다만 그 잣대를 자기 자신에게도 대야 한다** — 남의 번호를 반박할 때는 **NVD description 한 문장**을 읽어 컴포넌트가 내가 탄 경로와 같은지 확인하는 데 30초면 된다.
...
교과서적 Lua 웹셸이 그대로 실패했다 — **`io.popen`이 nil**이라 호출 단계에서 죽었다(표준 Lua 문구는 `attempt to call a nil value (field 'popen')`. 이 노트가 원래 인용했던 `field 'popen' is not callable`은 Lua 코어에 없는 문자열이라 삭제했다. 2-5 참조).
...
**놓칠 뻔한 이유**: 루트가 403이면 "인덱싱 금지 = 볼 것 없음"으로 반사적으로 넘어가게 된다. 실제로는 **autoindex만 꺼졌고 파일은 서빙**됐다. 게다가 `/data`·`/themes`·`/applications`·`/disk`는 **301(후행 슬래시 리다이렉트)** 로 응답했다 — 즉 **"이 디렉터리들이 실재한다"는 확정**까지 로그에 찍혀 있었다(1장 참조).

> [!warning] 확장자 목록이 좁았던 것은 맞다 — **그런데 그것만으로는 설명이 안 된다**
> 1차 feroxbuster를 `-x php,txt,html,lsp`로 돌렸고 **`conf`도 `dat`도 없었다.** 여기까지는 사실이다.
> **문제는 2차 스캔도 못 찾았다는 것이다.** `~/PG/Hub/ferox_80_files.log`를 보면 `-x conf,dat,zip,log,bak,txt`로 제대로 돌렸는데 결과가 `/`(403)·`/LICENSE.txt`·`/readme.txt` **세 줄**뿐이다.
>
...
>
> **여기서 가져갈 것 두 가지:**
> 1. **브루트포스 실패는 "확장자 부족"과 "워드리스트 부족"으로 나뉜다.** 후자는 `-x`로 안 풀린다 — **제품 지식·배포 아카이브·벤더 문서**로 가야 한다
> 2. **파일을 손에 넣었으면 "어떻게 찾았는지"를 그 자리에서 적어라.** 나중에 재구성하면 **가장 그럴듯한 경로(=재스캔)로 채워 넣게 된다**
>
...
**막혔다.** 다만 **어디서 어떻게 막혔는지**를 정확히 복원해둘 필요가 있다 — 이 노트는 원래 *"**구조를 알 수 없는 바이너리** → 즉시 손절 … 이 박스는 **세 번째**였다"* 라고 적어놨는데, **파일을 실제로 열어보면 바이너리가 아니다.**
...
**순수 ASCII JSON이고, 키 하나(`v`)에 base64 덩어리 하나가 들어 있다.** 아래 판정표 기준으로는 **두 번째 범주**(`base64/hex 덩어리 → 디코드해서 다시 판정`)다.
...
**즉 "두 번째로 시작해 세 번째로 귀결됐다."** 한 단계를 건너뛰고 "바이너리라서 손절"로 요약해버리면, 다음 박스에서 **JSON 껍데기를 보고도 "구조를 모르겠다"며 디코드를 안 해보게 된다.**

**어떻게 알아챘나**: base64를 풀어 알려진 해시 접두사가 하나도 없다는 것을 확인한 순간. **크랙을 시작하기 전이었다.**

> [!danger] `file`·`xxd`를 실제로 돌렸다면 나올 수 없는 서술이었다
> "제품 고유 암호화 포맷" 이라는 **결론은 맞았다.** 틀린 것은 **거기에 도달한 경로의 기록**이다.
> 결론이 맞으면 과정 기술의 오류는 잘 눈에 띄지 않는다 — **그래서 위험하다.** 학습 자료에서 값진 것은 결론이 아니라 **"어떤 관측을 보고 어떻게 분기했는가"** 인데, 그 부분이 통째로 잘못 적혀 있었다.
> **규칙: 판정표를 쓸 때는 "이 박스가 몇 번이었나"를 기억이 아니라 파일을 다시 열어서 적어라.**
...
> - **② base64/hex 덩어리** → **디코드해서 다시 판정**(①이나 ③으로 재분류된다)
> - **③ 구조를 알 수 없는 바이너리/blob** → **즉시 손절.** 제품 고유 포맷은 소스나 리버싱 없이는 못 푼다
>
> **이 박스는 ②로 시작해 ③으로 귀결됐다.** ②는 종착지가 아니라 **경유지**다 — 반드시 한 번 더 판정해야 한다.
> **여기서 "그래도 혹시" 하며 hashcat 모드를 하나씩 돌려봤다면 시간을 통째로 날렸을 것이다.**
> 그리고 `file`이 `JSON text data`를 뱉었다면 **첫 명령에서 이미 ②가 확정**된다. `file` 한 줄이 3초, 전체 판정이 30초다.
...
5. **확장자 목록을 2단으로 돌려라.** 1차는 언어 확장자(`php,html,jsp,aspx,lsp`)로 빠르게, **2차는 설정·데이터 확장자**(`conf,cfg,ini,env,json,yml,xml,dat,db,sql,bak,old,zip,log`)로 다시.
   - ⚠️ **그런데 2차도 비면 확장자가 아니라 워드리스트를 의심하라.** 이 박스의 `bdd.conf`·`user.dat`은 **확장자를 넓힌 2차 스캔에서도 안 나왔다** — `raft-medium-files.txt`에 그 파일명 자체가 없기 때문이다. 기저 단어가 없으면 `-x`는 아무 요청도 만들지 않는다.
...
10. **업로드 경로 ≠ 서빙 경로.** `PUT /fs/x.lsp` 로 올리고 **`GET /x.lsp`**(웹 루트 경로)로 요청해야 실행된다 — 이 박스에서 확인한 것은 이 조합이다. `GET /fs/x.lsp`가 무엇을 주는지는 **때려본 기록이 없다** `[가정 — 파일 매니저가 소스를 반환할 가능성이 높다]`. **업로드한 경로 그대로 GET해서 "실행 안 됨"이라고 결론 내리기 전에, 다른 경로로도 요청해봐라.**
...
17. **랩 브리핑·CVE 번호를 사실로 취급하지 마라 — 그리고 그 잣대를 자기 자신에게도 대라.** 이 박스는 브리핑이 8.1, 실행 버전이 8.4였다. 그런데 **"그럼 8.4용 CVE-2024-27697이 맞겠지"라고 이 노트가 붙인 대체 번호가 또 틀렸다** — 실제로 탄 것은 브리핑이 안내한 **CVE-2023-24078** 쪽이었다.
    - **버전 인접성은 취약점 동일성의 근거가 아니다.** 확인 비용은 **NVD description 한 문장**, 30초다 — *어느 컴포넌트인가*가 내가 탄 경로와 같은지만 보면 된다.
    - **CVE 번호가 아니라 메커니즘으로 검색**하라 — 시험에는 그 CVE가 안 나오고 **같은 유형**이 나온다. 이 박스는 번호를 두 번 틀리는 동안에도 체인이 계속 통했다.
...
- **CVE-2024-27697** — **이 노트의 경로와 무관한 별개 취약점.** FuguHub 8.4의 관리자 패널 About 페이지(`customize.lsp`)가 편집 가능한 Lua 페이지라는 점을 악용한다 `[가정 — NVD REST API 조회 결과 `totalResults: 0`(미공개/보류)이며, 위 설명은 공개 PoC 기준]`
  - 공개 PoC조차 *"계정 생성/인증에는 **CVE-2023-24078**을 leverage 하고 그 다음 `customize.lsp`"* 라고 **두 CVE를 구분해서** 쓴다
