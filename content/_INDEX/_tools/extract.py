...
def classify(rel):
    ...
    if p.startswith("03. PG"):
        # `_` 로 시작하는 문서(_STATUS·_PLAYBOOK·_WRITEUP-STANDARD·_HANDOFF)와
        # `_AUDIT\`·`_backup\` 하위는 «머신이 아니라 참조 문서»다.
        # 머신으로 분류하면 본문 키워드가 전부 기법으로 잡혀 기법별 색인의
        # 「실전 사례」에 등재된다 — 시험장 1차 진입점이 노이즈로 덮인다.
        if base.startswith("_") or "/_" in p:
            return "pg-index", "참조"
        return "pg", "머신"
    ...
    if p.startswith("OSCP-OS-"):
        return "pen200", "이론"
    # 05. PEN-200 — 교재 PDF 를 절 단위로 번역한 이론 노트. 머신이 아니다.
    # OSCP-OS- 와 절 번호 체계가 같지만 그쪽은 「연습문제 답·스크린샷」이고
    # 이쪽은 「교재 본문」이라 플랫폼을 갈라 둔다.
    if p.startswith("05. PEN-200"):
        return "pen200-book", "이론"
...
TECH_RAW = [
    ...
    ("tech/ad/gmsa",             [r"gMSA", r"msDS-ManagedPassword", r"ReadGMSAPassword", r"gmsadumper"]),
    ("tech/ad/coerced-auth",     [r"PetitPotam", r"Coercer", r"printerbug", r"EfsRpc", r"coerce"]),
...
MANUAL_SVC_RE = re.compile(r"^manual_services:\s*true\s*(?:#.*)?$", re.M | re.I)
DECL_SVC_RE = re.compile(r"^services:\s*\[([^\]]*)\]\s*$", re.M | re.I)
MANUAL_PORT_RE = re.compile(r"^manual_ports:\s*true\s*(?:#.*)?$", re.M | re.I)
DECL_PORT_RE = re.compile(r"^ports:\s*\[([^\]]*)\]\s*$", re.M | re.I)
...
def declared_services(text):
    """노트가 `manual_services: true` 를 선언했으면 프론트매터의 services 를 그대로 쓴다.

    `PORT_RE` 가 긁는 nmap raw 에는 **오탐 서비스명**이 섞인다 — 실측(2026-08-24
    MiddlewareBypass): `ppp` 는 nmap 오탐이고 노트 본문이 스스로 그것을 반박하는데도
    색인에는 그대로 올라갔다. `manual_ports` 와 같은 취지의 장치다.
    """
    fm = frontmatter(text)
    if fm is None or not MANUAL_SVC_RE.search(fm):
        return None
    m = DECL_SVC_RE.search(fm)
    if not m:
        return []
...
def declared_ports(text):
    """노트가 `manual_ports: true` 를 선언했으면 프론트매터의 ports 를 그대로 쓴다.

    `PORT_RE` 는 본문 **어디에 있든** `443/tcp open https` 형태를 긁는다. 그런데 노트는
    §「nmap raw 출력을 압축·생략하지 마라」 규율에 따라 **오탐이 섞인 스캔 원문도 그대로**
    싣는다 — 실측(2026-08-24 Muddy): 443·808·908 이 오탐인데 raw 를 보존했다는 이유로
    색인에 올라갔다. 「raw 보존」과 「오탐 미색인」이 도구 없이는 양립하지 않는다.

    그래서 `manual_tags`·`manual_cves` 와 같은 장치를 포트에도 둔다. 사람이 선언하면
    본문 스캔을 건너뛴다. ⚠️ 선언 줄 뒤 주석은 안전하지만 **`ports: [...]` 값 줄 뒤에
    주석을 붙이면 정규식이 실패해 포트가 통째로 사라진다**(§5 와 같은 함정).
    """
    fm = frontmatter(text)
    if fm is None or not MANUAL_PORT_RE.search(fm):
        return None
    m = DECL_PORT_RE.search(fm)
    if not m:
        return []
    out = []
...
def extract(path, rel):
    ...
    # `microsoft windows [version` 는 Windows 배너의 확정 지문이다(2026-08-24 추가).
    # 실측: 이 마커 없이는 Robust 가 os 미판정으로 남아 OS 별 색인에서 사라졌다.
    # 후보 5종을 오탐 검사한 결과 «리눅스로 판정된 노트에서 0건» 인 것은 이것뿐이라 이것만 넣는다
    # (`nt authority`·`ms-wbt-server`·`whoami /all` 은 리눅스 노트에서도 등장했다).
    win = len(re.findall(r"cpe:/o:microsoft|running:\s*microsoft windows|os:\s*windows|microsoft windows rpc"
                         r"|microsoft windows \[version", low))
    ...
    for p, _proto, svc in PORT_RE.findall(body):
        ...
        if s not in ("unknown", "tcpwrapped"):
            svcs.add(s)
    decl_ports = declared_ports(text)
    if decl_ports is not None:
        if decl_ports:
            meta["ports"] = decl_ports
        meta["manual_ports"] = True
    elif ports:
        meta["ports"] = sorted(ports)
    # nmap 이 같은 서비스를 판본에 따라 다른 이름으로 뱉는 경우가 있다.
    # 그대로 두면 색인에서 같은 서비스가 둘로 갈려 「이 서비스를 만난 박스」가 반쪽만 나온다.
    # 실측(2026-08-24): Wombo 27017 이 `mongod` 와 `mongodb` 로 동시에 등재됐다.
    # ⚠️ 접두가 같다고 다 별칭은 아니다 — `http`/`https`, `ldap`/`ldapssl`,
    # `mysql`/`mysqlx` 는 실제로 다른 서비스라 합치면 안 된다. 확인된 것만 넣는다.
    SVC_ALIAS = {"mongod": "mongodb"}
    svcs = {SVC_ALIAS.get(s, s) for s in svcs}
    decl_svcs = declared_services(text)
    if decl_svcs is not None:
        if decl_svcs:
            meta["services"] = decl_svcs
        meta["manual_services"] = True
    elif svcs:
    ...
    if meta.get("platform") == "pg-index":
        # `03. PG\_*` 색인 문서(_PLAYBOOK·_STATUS·_WRITEUP-STANDARD·_HANDOFF)는
        # 여러 박스의 CVE 를 «인용»한다 — 그중엔 「이 박스가 아니다」라고 반증하려
        # 적은 번호도 있다. CVE 색인은 「그 CVE 로 뚫은 머신」을 찾는 곳이므로
        # 인용 문서가 끼면 오답이 된다. manual_cves 로는 못 막는다 —
        # inject.py 의 `ours` 판정이 그 키를 보고 손으로 쓴 프론트매터를 재작성한다.
        pass
    elif decl_cves is not None:
        if decl_cves:
            meta["cves"] = decl_cves
