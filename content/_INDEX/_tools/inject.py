...
def build_fm(m):
    ...
    if m.get("manual_ports"):
        # 포트도 사람이 관리한다. `PORT_RE` 는 본문 어디에 있든 nmap raw 를 긁는데,
        # 노트는 「raw 를 압축·생략하지 마라」 규율 때문에 **오탐이 섞인 스캔 원문도** 싣는다.
        # ⚠️ 이 줄을 빼면 선언이 사라져 다음 refresh 때 오탐이 되살아난다 — 실제로 났다(2026-08-24).
        lines.append("manual_ports: true")
    if m.get("manual_services"):
        # 서비스명도 같은 이유로 사람이 관리한다(MiddlewareBypass 의 `ppp` 오탐).
        lines.append("manual_services: true")
