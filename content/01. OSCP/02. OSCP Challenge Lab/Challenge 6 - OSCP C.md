...
이 실습은 노출된 웹 서버의 SQLite 데이터베이스에서 자격 증명을 찾는 것부터 시작하여 Active Directory 공격 과정을 안내합니다. 자격 증명을 크랙하여 WinRM을 통해 내부 시스템에 접근하고, 바이너리 분석 및 피벗팅을 통해 권한을 상승시킨 후, 도메인 관리자 해시를 추출하여 도메인 전체를 장악하는 방법을 보여줍니다.
...
이 실습은 학습자가 실제 Active Directory 공격 시나리오에 몰입할 수 있도록 구성되어 있으며, 취약한 서비스를 식별하는 것부터 시작하여 도메인 전체를 침해하는 단계로 진행됩니다. 참가자는 웹 서버에서 노출된 SQLite 데이터베이스를 발견하고, 자격 증명을 추출 및 크랙하여 WinRM을 통해 내부 시스템에 접근하는 것으로 시작합니다. 상세한 바이너리 분석과 권한 상승을 통해 관리자 자격 증명을 확보하고, Chisel과 같은 터널링 도구를 사용하여 네트워크 깊숙이 침투합니다.

공격 과정이 진행됨에 따라 참여자들은 도메인 관리자 자격 증명을 추출하고 도메인 컨트롤러에 접근하여 증거 파일을 확보함으로써 도메인 지배권을 획득하며, 사후 공격 및 측면 이동 기술에 대한 숙련도를 입증합니다.

침투 테스터, 레드팀, 보안 전문가에게 이상적인 이 실습 환경은 서비스 열거, 자격 증명 추출 및 크래킹, 바이너리 공격, 터널링, Active Directory 침해와 같은 핵심 기술을 습득하는 데 도움을 줍니다. 공격자가 네트워크를 통해 어떻게 이동하여 도메인 전체를 장악하는지 직접 체험하며 처음부터 끝까지 살펴볼 수 있습니다.
...
- 웹 서버의 SQLite 데이터베이스를 포함하여 취약한 엔드포인트를 식별하기 위해 노출된 서비스를 열거합니다.
- 데이터베이스에서 자격 증명을 추출하고 해독하여, 이를 활용해 WinRM을 통해 내부 시스템에 인증합니다.
- 바이너리 파일을 분석하고 악용하여 권한 상승을 시도하고 관리자 자격 증명을 탈취합니다.
- Chisel과 같은 터널링 도구를 구성하고 사용하여 추가 네트워크 세그먼트로 연결하십시오.
- 도메인 관리자 자격 증명을 추출하고 도메인 컨트롤러에 로그인하여 증명 파일을 검색함으로써 도메인 우위를 확보하십시오.
